...
# ── Thư mục dữ liệu ──────────────────────────────────────────────────────────
EXCEL_DIR = "./data/excel"   # để file .xlsx / .csv vào đây
PDF_DIR   = "./data/pdf"     # để file .pdf vào đây
...
def _tai_embedding_model():
    """Tải embedding model (lần đầu tải ~1.1GB, các lần sau dùng cache)."""
    log.info(f"Đang tải embedding model: {EMBEDDING_MODEL}")
    log.info("Lần đầu chạy sẽ tải model (~1.1GB). Các lần sau rất nhanh.")
    model = SentenceTransformer(EMBEDDING_MODEL)
    log.info("Tải model xong!")
    return model
...
def _luu_vao_chroma(collection, embed_model, documents, metadatas, ids, ten_nguon):
    """Embed documents rồi lưu vào ChromaDB theo batch 64."""
    if not documents:
        return
    BATCH = 64
    ...
    for i in tqdm(range(0, len(documents), BATCH), desc="  Embed", unit="batch"):
        ...
        texts      = [f"passage: {d}" for d in batch_docs]  # prefix chuẩn e5
...
def nap_excel(collection, embed_model):
    ...
    for filename in files:
        ...
        _luu_vao_chroma(collection, embed_model, documents, metadatas, ids, filename)


def nap_pdf(collection, embed_model):
...
def nap_web(collection, embed_model):
    ...
    for site in WEBSITES_TO_CRAWL:
        ...
        metadatas = [{"nguon": "web", "url": url, "truong": truong,
                     ...
                     for i in range(len(chunks))]
        ids = [str(uuid.uuid4()) for _ in chunks]
        _luu_vao_chroma(collection, embed_model, documents, metadatas, ids, url)
        time.sleep(1)
...
def chay_ingest():
    ...
    embed_model = _tai_embedding_model()
    os.makedirs(EXCEL_DIR, exist_ok=True)
    os.makedirs(PDF_DIR,   exist_ok=True)

    print("\n--- Nạp từ Excel/CSV ---")
    nap_excel(collection, embed_model)
    print("\n--- Nạp từ PDF ---")
    nap_pdf(collection, embed_model)
    print("\n--- Nạp từ Website ---")
    nap_web(collection, embed_model)
